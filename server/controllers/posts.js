...
export const getFeedPosts = async (req, res) => {
    try {
        const post = await Post.find().sort({ createdAt: -1 }); // Sorting by 'createdAt' in reverse order
...
export const getUserPosts = async (req, res) => {
    try {
        ...
        const post = await Post.find({ userId }).sort({ createdAt: -1 }); // Sorting by 'createdAt' in reverse order;
        res.status(200).json(post);
    } catch (err) {
        res.status(404).json({ message: err.message });
    }
